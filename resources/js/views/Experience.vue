<template>
    <section class="section">
        <div class="container">
            <h2 class="title is-2">Experience</h2>
            <div class="timeline">
                <header class="timeline-header">
                    <span class="tag is-medium is-primary">Now</span>
                </header>
                <div class="timeline-item">
                    <div class="timeline-marker is-image is-32x32">
<!--                        <img src="http://bulma.io/images/placeholders/32x32.png">-->
                    </div>
                    <div class="timeline-content">
                        <p class="heading">2014 - 2017</p>
                        <p class="is-size-4" style="margin-bottom:1.5rem;">
                            <strong>Inetvi</strong> - a small team in venezuela, continuing working with South and North american clients, providing business management systems.
                        </p>

                        <p class="heading">Languages Used</p>
                        <div class="level">
                            <div class="level-left">
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'php']"></font-awesome-icon>
                                    </span>
                                    <span>PHP</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'html5']"></font-awesome-icon>
                                    </span>
                                    <span>HTML</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'mysql']"></font-awesome-icon>
                                    </span>
                                    <span>MySQL</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'css3-alt']"></font-awesome-icon>
                                    </span>
                                    <span>CSS</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'js']"></font-awesome-icon>
                                    </span>
                                    <span>JS</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'elasticsearch']"></font-awesome-icon>
                                    </span>
                                    <span>ElasticSearch</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'elasticsearch']"></font-awesome-icon>
                                    </span>
                                    <span>Redis</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'influxdb']"></font-awesome-icon>
                                    </span>
                                    <span>InfluxDB</span>
                                </div>
                            </div>
                        </div>

                        <h4 class="heading">Frameworks Used</h4>
                        <div class="level">
                            <div class="level-left">
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'laravel']"></font-awesome-icon>
                                    </span>
                                    <span>Laravel</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'vuejs']"></font-awesome-icon>
                                    </span>
                                    <span>VueJS</span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <header class="timeline-header">
                    <span class="tag is-primary">2014</span>
                </header>
                <div class="timeline-item">
                    <div class="timeline-marker is-image is-32x32">
<!--                        <img src="http://bulma.io/images/placeholders/32x32.png">-->
                    </div>
                    <div class="timeline-content">
                        <p class="heading">2008 - 2014</p>
                        <p class="is-size-4" style="margin-bottom:1.5rem;"><strong>Freelancing</strong> - working as a remote freelancer with clients from Venezuela and USA</p>

                        <p class="heading">Languages Used</p>
                        <div class="level">
                            <div class="level-left">
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'php']"></font-awesome-icon>
                                    </span>
                                    <span>PHP</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'html5']"></font-awesome-icon>
                                    </span>
                                    <span>HTML</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'mysql']"></font-awesome-icon>
                                    </span>
                                    <span>MySQL</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'css3-alt']"></font-awesome-icon>
                                    </span>
                                    <span>CSS</span>
                                </div>
                                <div class="level-item justify-start">
                                    <span class="icon">
                                        <font-awesome-icon :icon="['fab', 'js']"></font-awesome-icon>
                                    </span>
                                    <span>JS</span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="timeline-header">
                    <span class="tag is-medium is-primary">2008</span>
                </div>
            </div>
        </div>
    </section>
</template>

<script>
    export default {
        metaInfo: {
            title: 'El Suterino',
            titleTemplate: '%s - Experience / Skills',
        },
        data() {
            return {
                skills: {
                    languages: [
                        {
                            name: 'PHP',
                            level: 5,
                            expertise: 10,
                            icon: ['fab', 'php']
                        },
                        {
                            name: 'JavaScript',
                            level: 5,
                            expertise: 8,
                            icon: ['fab', 'js']
                        },
                        {
                            name: 'HTML',
                            level: 5,
                            expertise: 10,
                            icon: ['fab', 'html5']
                        },
                        {
                            name: 'CSS',
                            level: 5,
                            expertise: 8,
                            icon: ['fab', 'css3-alt']
                        },
                        {
                            name: 'SQL',
                            level: 5,
                            expertise: 10,
                            icon: ['fas', 'database']
                        }
                    ],
                    frameworks: [
                        {
                            name: 'Laravel',
                            level: 5,
                            expertise: 5,
                            icon: ['fab', 'laravel']
                        },
                        {
                            name: 'VueJS',
                            level: 5,
                            expertise: 3,
                            icon: ['fab', 'vuejs']
                        },
                        {
                            name: 'Symfony',
                            level: 4,
                            expertise: 2,
                            icon: ['fab', 'symfony']
                        },
                        {
                            name: 'JQuery',
                            level: 3,
                            expertise: 3,
                            icon: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="svg-inline--fa fa-symfony fa-w-16" baseProfile="tiny" height="512px" version="1.1" viewBox="0 0 512 512" width="512px" xml:space="preserve"><g id="Layer_6"><g><path d="M34.317,126.054c-44.877,64.496-39.286,148.409-5.01,216.945c0.816,1.644,1.67,3.259,2.513,4.88    c0.537,1.02,1.044,2.072,1.603,3.08c0.314,0.607,0.669,1.199,0.998,1.779c0.586,1.095,1.182,2.131,1.79,3.199    c1.065,1.876,2.158,3.741,3.272,5.606c0.618,1.025,1.228,2.05,1.865,3.074c1.236,1.996,2.502,3.976,3.799,5.943    c0.54,0.841,1.065,1.682,1.619,2.494c1.787,2.668,3.614,5.331,5.498,7.944c0.049,0.065,0.103,0.136,0.158,0.201    c0.298,0.423,0.629,0.835,0.93,1.253c1.629,2.25,3.313,4.468,5.01,6.669c0.626,0.798,1.261,1.6,1.89,2.402    c1.516,1.909,3.061,3.802,4.631,5.693c0.588,0.705,1.177,1.415,1.771,2.109c2.106,2.479,4.256,4.951,6.456,7.375    c0.038,0.044,0.089,0.093,0.13,0.136c0.089,0.103,0.168,0.168,0.247,0.266c2.145,2.343,4.338,4.637,6.562,6.925    c0.688,0.704,1.394,1.415,2.101,2.12c1.722,1.729,3.471,3.427,5.238,5.135c0.718,0.678,1.429,1.377,2.142,2.033    c2.361,2.224,4.745,4.403,7.177,6.529c0.043,0.043,0.073,0.076,0.114,0.092c0.412,0.369,0.84,0.722,1.247,1.063    c2.145,1.859,4.325,3.703,6.521,5.515c0.895,0.721,1.8,1.437,2.701,2.152c1.795,1.438,3.608,2.853,5.436,4.246    c0.965,0.737,1.933,1.475,2.906,2.213c2.015,1.475,4.045,2.943,6.098,4.37c0.749,0.542,1.472,1.062,2.224,1.578    c0.206,0.146,0.396,0.298,0.607,0.438c1.947,1.334,3.934,2.631,5.921,3.916c0.854,0.568,1.703,1.154,2.565,1.707    c3.05,1.926,6.144,3.834,9.272,5.684c0.852,0.482,1.708,0.96,2.562,1.442c2.299,1.328,4.612,2.63,6.96,3.904    c1.266,0.683,2.57,1.328,3.847,1.979c1.659,0.873,3.302,1.751,4.983,2.586c0.382,0.195,0.778,0.38,1.155,0.559    c0.672,0.325,1.355,0.646,2.036,0.987c2.608,1.247,5.238,2.467,7.89,3.654c0.567,0.249,1.112,0.499,1.673,0.743    c3.04,1.328,6.106,2.613,9.208,3.834c0.743,0.298,1.494,0.585,2.243,0.884c2.866,1.101,5.77,2.19,8.684,3.216    c0.371,0.13,0.727,0.249,1.092,0.373c3.202,1.123,6.434,2.158,9.69,3.173c0.775,0.228,1.556,0.478,2.337,0.705    c3.313,0.997,6.583,2.185,10.005,2.819c217.021,39.574,280.061-130.418,280.061-130.418    c-52.942,68.976-146.924,87.174-235.967,66.914c-3.376-0.759-6.646-1.811-9.938-2.787c-0.837-0.249-1.656-0.498-2.48-0.753    c-3.213-0.987-6.399-2.022-9.568-3.112c-0.434-0.152-0.873-0.314-1.315-0.473c-2.838-0.997-5.642-2.049-8.438-3.129    c-0.789-0.314-1.578-0.612-2.364-0.927c-3.063-1.209-6.108-2.473-9.11-3.79c-0.621-0.261-1.212-0.537-1.819-0.809    c-2.583-1.166-5.138-2.342-7.689-3.568c-0.732-0.357-1.47-0.704-2.218-1.067c-1.996-0.971-3.975-2.007-5.943-3.021    c-1.32-0.694-2.662-1.355-3.966-2.077c-2.408-1.29-4.772-2.63-7.123-3.996c-0.8-0.45-1.605-0.879-2.408-1.34    c-3.129-1.843-6.22-3.752-9.278-5.688c-0.843-0.536-1.67-1.111-2.502-1.664c-2.218-1.432-4.428-2.885-6.6-4.393    c-0.721-0.477-1.42-1.008-2.128-1.491c-2.102-1.48-4.178-2.987-6.242-4.511c-0.933-0.689-1.857-1.405-2.765-2.094    c-1.89-1.447-3.771-2.9-5.631-4.403c-0.838-0.667-1.681-1.339-2.511-2.012c-2.367-1.946-4.704-3.898-7.014-5.905    c-0.241-0.217-0.504-0.423-0.767-0.645c-2.495-2.196-4.951-4.447-7.372-6.708c-0.705-0.672-1.388-1.323-2.077-2.001    c-1.797-1.702-3.562-3.427-5.297-5.189c-0.694-0.694-1.394-1.383-2.077-2.071c-2.209-2.267-4.376-4.555-6.501-6.881    c-0.106-0.114-0.217-0.217-0.32-0.342c-2.248-2.457-4.444-4.967-6.594-7.505c-0.583-0.694-1.149-1.366-1.727-2.05    c-1.602-1.931-3.191-3.889-4.742-5.867c-0.586-0.705-1.168-1.438-1.744-2.164c-1.865-2.424-3.692-4.836-5.482-7.299    C47.589,251.017,29.828,158.009,69.339,81.627"/><path d="M173.617,71.714c-32.454,46.705-30.691,109.215-5.376,158.61c4.246,8.28,9.018,16.311,14.356,23.962    c4.861,6.968,10.249,15.259,16.701,20.854c2.343,2.576,4.786,5.092,7.283,7.56c0.632,0.651,1.277,1.264,1.922,1.903    c2.421,2.343,4.888,4.658,7.437,6.903c0.105,0.087,0.198,0.189,0.304,0.26c0.027,0.032,0.054,0.044,0.07,0.076    c2.82,2.468,5.731,4.848,8.689,7.185c0.656,0.499,1.307,1.025,1.971,1.54c2.963,2.272,5.987,4.512,9.088,6.638    c0.092,0.064,0.171,0.13,0.263,0.206c1.367,0.938,2.763,1.821,4.146,2.728c0.667,0.434,1.299,0.895,1.969,1.301    c2.212,1.416,4.455,2.799,6.735,4.127c0.322,0.201,0.64,0.363,0.952,0.548c1.965,1.133,3.958,2.25,5.957,3.329    c0.697,0.396,1.407,0.737,2.118,1.106c1.38,0.722,2.76,1.453,4.167,2.158c0.203,0.098,0.423,0.195,0.626,0.282    c2.86,1.409,5.748,2.766,8.692,4.045c0.627,0.276,1.277,0.525,1.92,0.791c2.351,0.998,4.731,1.975,7.117,2.891    c1.014,0.385,2.039,0.748,3.053,1.123c2.166,0.803,4.347,1.539,6.54,2.277c0.992,0.314,1.965,0.64,2.958,0.943    c3.12,0.965,6.203,2.19,9.456,2.732c167.56,27.764,206.247-101.25,206.247-101.25c-34.875,50.212-102.405,74.186-174.468,55.478    c-3.199-0.84-6.353-1.757-9.486-2.744c-0.96-0.292-1.903-0.607-2.853-0.922c-2.229-0.721-4.444-1.49-6.64-2.287    c-1.006-0.364-2.01-0.732-2.996-1.112c-2.408-0.927-4.786-1.882-7.145-2.885c-0.643-0.276-1.296-0.521-1.925-0.813    c-2.958-1.285-5.87-2.646-8.744-4.056c-1.478-0.728-2.928-1.491-4.381-2.234c-0.841-0.439-1.687-0.873-2.53-1.307    c-1.868-1.015-3.72-2.061-5.55-3.129c-0.431-0.249-0.884-0.482-1.323-0.749c-2.271-1.328-4.506-2.694-6.719-4.099    c-0.683-0.439-1.339-0.911-2.022-1.356c-1.461-0.948-2.92-1.908-4.352-2.879c-3.088-2.125-6.095-4.359-9.056-6.626    c-0.683-0.537-1.369-1.063-2.039-1.6c-31.546-24.906-56.543-58.943-68.43-97.53c-12.458-40.029-9.776-84.95,11.808-121.411"/><path d="M290.774,31.116c-19.131,28.154-21.01,63.119-7.735,94.217c13.998,33.007,42.684,58.9,76.168,71.171    c1.38,0.515,2.757,0.976,4.159,1.448c0.609,0.19,1.214,0.396,1.832,0.575c1.972,0.623,3.932,1.345,5.971,1.729    c92.52,17.878,117.615-47.48,124.296-57.1c-21.986,31.651-58.925,39.249-104.254,28.252c-3.582-0.879-7.519-2.169-10.968-3.395    c-4.425-1.573-8.784-3.373-13.024-5.406c-8.047-3.867-15.718-8.562-22.835-13.931c-40.558-30.778-65.759-89.488-39.291-137.305"/></g></g></svg>'
                        },
                        {
                            name: 'Wordpress',
                            level: 2,
                            expertise: 1,
                            icon: ['fab', 'wordpress']
                        },
                    ],
                    databases: [
                        {
                            name: 'MySQL',
                            level: 5,
                            expertise: 10,
                            icon: '<svg xmlns="http://www.w3.org/2000/svg" role="img" viewBox="0 0 24 24" class="svg-inline--fa fa-symfony fa-w-16"><title/><path d="M16.405 5.676c-.115 0-.193.014-.274.033v.013h.014c.054.104.146.18.214.273.054.107.1.214.154.32l.014-.015c.094-.066.14-.172.14-.333-.04-.047-.046-.094-.08-.14-.04-.067-.126-.1-.18-.153zM5.77 18.87h-.927c-.03-1.562-.123-3.03-.27-4.41h-.008l-1.41 4.41H2.45l-1.4-4.41h-.01c-.103 1.323-.168 2.793-.195 4.41H0c.055-1.966.192-3.81.41-5.53h1.15l1.335 4.064h.008L4.25 13.34h1.095c.242 2.015.384 3.86.428 5.53zm4.017-4.08c-.378 2.045-.876 3.533-1.492 4.46-.482.716-1.01 1.073-1.583 1.073-.153 0-.34-.046-.566-.138v-.494c.11.017.24.026.386.026.268 0 .483-.075.647-.222.197-.18.295-.382.295-.605 0-.155-.077-.47-.23-.944L6.23 14.79h.91l.727 2.36c.164.536.233.91.205 1.123.4-1.064.678-2.227.835-3.483h.88zM22.112 18.87h-2.63v-5.53h.885v4.85h1.745zM18.792 19.005l-1.016-.5c.09-.076.177-.158.255-.25.433-.506.648-1.258.648-2.253 0-1.83-.718-2.746-2.155-2.746-.704 0-1.254.232-1.65.697-.43.508-.646 1.256-.646 2.245 0 .972.19 1.686.574 2.14.35.41.877.615 1.583.615.264 0 .506-.033.725-.098l1.325.772.36-.622zM15.5 17.763c-.225-.36-.337-.94-.337-1.736 0-1.393.424-2.09 1.27-2.09.443 0 .77.167.977.5.224.362.336.936.336 1.723 0 1.404-.424 2.108-1.27 2.108-.445 0-.77-.167-.978-.5zM13.842 17.338c0 .47-.172.856-.516 1.156s-.803.45-1.384.45c-.543 0-1.064-.172-1.573-.515l.237-.476c.438.22.833.328 1.19.328.332 0 .593-.073.783-.22.188-.147.3-.354.3-.615 0-.33-.23-.61-.648-.845-.388-.213-1.163-.657-1.163-.657-.422-.307-.632-.636-.632-1.177 0-.45.157-.81.47-1.085.315-.278.72-.415 1.22-.415.512 0 .98.136 1.4.41l-.213.476c-.36-.152-.715-.23-1.064-.23-.283 0-.502.068-.654.206-.153.136-.248.31-.248.524 0 .328.234.61.666.85.393.215 1.187.67 1.187.67.433.305.648.63.648 1.168zM23.224 11.486c-.535-.014-.95.04-1.297.188-.1.04-.26.04-.274.167.055.053.063.14.11.214.08.134.218.313.346.407.14.11.28.216.427.31.26.16.555.255.81.416.145.094.293.213.44.313.073.05.12.14.214.172v-.02c-.046-.06-.06-.147-.105-.214-.067-.067-.134-.127-.2-.193-.194-.26-.435-.487-.695-.675-.214-.146-.682-.35-.77-.595l-.013-.014c.146-.013.32-.066.46-.106.227-.06.435-.047.67-.106.106-.027.213-.06.32-.094v-.06c-.12-.12-.21-.283-.334-.395-.34-.295-.717-.582-1.104-.823-.21-.134-.476-.22-.697-.334-.08-.04-.214-.06-.26-.127-.12-.146-.19-.34-.275-.514-.192-.368-.38-.775-.547-1.163-.12-.262-.193-.523-.34-.763-.69-1.137-1.437-1.826-2.586-2.5-.247-.14-.543-.2-.856-.274-.167-.008-.334-.02-.5-.027-.11-.047-.216-.174-.31-.235-.38-.24-1.364-.76-1.644-.072-.18.434.267.862.422 1.082.115.153.26.328.34.5.047.116.06.235.107.356.106.294.207.622.347.897.073.14.153.287.247.413.054.073.146.107.167.227-.094.136-.1.334-.154.5-.24.757-.146 1.693.194 2.25.107.166.362.534.703.393.3-.12.234-.5.32-.835.02-.08.007-.133.048-.187v.015c.094.188.188.367.274.555.206.328.566.668.867.895.16.12.287.328.487.402v-.02h-.015c-.043-.058-.1-.086-.154-.133-.12-.12-.255-.267-.35-.4-.28-.377-.527-.79-.747-1.218-.11-.21-.202-.436-.29-.643-.04-.08-.04-.2-.107-.24-.1.146-.247.273-.32.453-.127.288-.14.642-.188 1.01-.027.007-.014 0-.027.014-.214-.052-.287-.274-.367-.46-.2-.475-.233-1.238-.06-1.785.047-.14.247-.582.167-.716-.042-.127-.174-.2-.247-.303-.087-.124-.18-.285-.24-.427-.16-.374-.24-.788-.414-1.162-.08-.173-.22-.354-.334-.513-.127-.18-.267-.307-.368-.52-.033-.073-.08-.194-.027-.274.014-.054.042-.075.094-.09.088-.072.335.022.422.062.247.1.455.194.662.334.094.066.195.193.315.226h.14c.214.047.455.014.655.073.355.114.675.28.962.46.876.556 1.596 1.345 2.085 2.286.08.154.115.295.188.455.14.33.313.663.455.982.14.315.275.636.476.897.1.14.502.213.682.286.133.06.34.115.46.188.23.14.454.3.67.454.11.076.443.243.463.378z"/></svg>'
                        },
                        {
                            name: 'ElasticSearch',
                            level: 3,
                            expertise: 2,
                            icon: '<svg xmlns="http://www.w3.org/2000/svg" role="img" viewBox="0 0 24 24" class="svg-inline--fa fa-symfony fa-w-16"><title/><path d="M11.27 15.58H1.61A11.81 11.81 0 0 1 1.07 12c0-1.25.18-2.45.54-3.58h15.86A3.57 3.57 0 0 1 21.05 12a3.56 3.56 0 0 1-3.55 3.58zm-.45 1.2H2.05a11.8 11.8 0 0 0 4.39 5.18A11.8 11.8 0 0 0 13 23.93c4.15 0 7.78-2.12 9.93-5.3a6.08 6.08 0 0 0-4.35-1.86zm7.76-9.55c1.7 0 3.25-.72 4.35-1.85A11.96 11.96 0 0 0 2.05 7.23h8.74z"/></svg>'
                        },
                        {
                            name: 'InfluxDB',
                            level: 4,
                            expertise: 3,
                            icon: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" width="238px" height="244px" viewBox="104 0 28 168"  class="svg-inline--fa fa-symfony fa-w-16" xml:space="preserve"><g><path class="st1" d="M199.6,101l-15.9-69.3c-0.9-3.8-4.4-7.6-8.1-8.6L102.8,0.7c-0.9-0.3-2-0.5-3-0.5c-3.2,0-6.4,1.2-8.5,3L39,51.7   c-2.7,2.6-4.4,7.6-3.5,11.2l17,74.2c0.9,3.8,4.4,7.6,8.1,8.6l68.1,20.9c0.9,0.3,2,0.5,3,0.5c3.2,0,6.4-1.2,8.5-3l55.9-51.7   C199,109.6,200.5,104.8,199.6,101 M110.2,17.2l49.8,15.3c2,0.6,2,1.5,0,2l-26.3,6.1c-2,0.5-4.7-0.5-6.1-1.8l-18.4-19.7   C107.8,17.4,108.3,16.6,110.2,17.2 M141.4,106.7c0.5,2-0.8,3.2-2.7,2.6L84.7,92.6c-2-0.6-2.3-2.3-0.8-3.6l41.3-38.2   c1.5-1.4,3-0.9,3.5,1.1L141.4,106.7z M52.5,57.7l43.8-40.7c1.5-1.4,3.8-1.4,5.3,0.2l22,23.7c1.4,1.5,1.4,3.8-0.2,5.3L79.6,86.9   c-1.5,1.4-3.8,1.4-5.3-0.2L52.4,63C51,61.5,51.1,59.1,52.5,57.7 M63.1,122.3L51.6,71.5c-0.5-2,0.3-2.4,1.7-0.9l18.4,19.7   c1.4,1.5,2,4.3,1.4,6.2l-7.9,25.8C64.5,124.3,63.6,124.3,63.1,122.3 M127.3,151.9l-57.1-17.6c-2-0.6-3-2.7-2.4-4.7l9.6-30.8   c0.6-2,2.7-3,4.7-2.4l57,17.6c2,0.6,3,2.7,2.4,4.7l-9.6,30.8C131.4,151.5,129.2,152.5,127.3,151.9 M178,110.5l-38.3,35.3   c-1.5,1.4-2.3,0.9-1.7-1.1L146,119c0.6-2,2.7-3.9,4.7-4.4l26.3-6.1C179.1,108.2,179.5,109,178,110.5 M182.3,102.9l-31.4,7.1   c-2,0.5-4-0.8-4.4-2.7L133,49.2c-0.5-2,0.8-3.9,2.7-4.4l31.4-7.1c2-0.4,3.9,0.8,4.4,2.7L185,98.5   C185.5,100.5,184.2,102.5,182.3,102.9"/> </g> </svg>'
                        },
                        {
                            name: 'Redis',
                            level: 4,
                            expertise: 3,
                            icon: '<svg xmlns="http://www.w3.org/2000/svg" role="img" viewBox="0 0 24 24" class="svg-inline--fa fa-symfony fa-w-16"><title/><path d="M23.99414 14.34005c-.01.229-.313.485-.935.81-1.281.667-7.916 3.396-9.328 4.132-1.413.736-2.197.729-3.313.195-1.116-.533-8.176-3.386-9.448-3.993-.635-.304-.959-.56-.97-.802v2.426c0 .242.334.499.97.803 1.272.608 8.333 3.46 9.448 3.993 1.116.534 1.9.541 3.313-.196 1.412-.736 8.047-3.464 9.328-4.132.651-.339.939-.603.939-.842 0-.226.001-2.392.001-2.392-.001-.001-.004-.001-.005-.002zM23.99314 10.38505c-.011.229-.313.484-.934.809-1.281.667-7.916 3.396-9.328 4.132-1.413.736-2.197.729-3.313.196-1.116-.533-8.176-3.386-9.448-3.994-.635-.303-.959-.56-.97-.801v2.426c0 .242.334.498.97.802 1.272.608 8.332 3.46 9.448 3.993 1.116.534 1.9.541 3.313-.195 1.412-.736 8.047-3.465 9.328-4.132.651-.34.939-.604.939-.843 0-.226.001-2.392.001-2.392-.001-.001-.004 0-.006-.001zM23.99314 6.28305c.012-.244-.307-.458-.949-.694-1.248-.457-7.843-3.082-9.106-3.545-1.263-.462-1.777-.443-3.261.089-1.484.533-8.506 3.287-9.755 3.776-.625.246-.931.473-.92.715v2.426c0 .242.334.498.97.802 1.272.608 8.332 3.461 9.448 3.994 1.116.533 1.9.54 3.313-.196 1.412-.736 8.047-3.465 9.328-4.132.651-.34.939-.604.939-.843 0-.225.001-2.392.001-2.392h-.008zm-15.399 2.296l5.561-.854-1.68 2.463-3.881-1.609zm12.299-2.218l-3.288 1.299-.357.14-3.287-1.299 3.642-1.44 3.29 1.3zm-9.655-2.383l-.538-.992 1.678.656 1.582-.518-.428 1.025 1.612.604-2.079.216-.466 1.12-.752-1.249-2.401-.216 1.792-.646zm-4.143 1.399c1.642 0 2.972.516 2.972 1.152 0 .636-1.331 1.152-2.972 1.152s-2.973-.517-2.973-1.152c0-.636 1.331-1.152 2.973-1.152z"/></svg>'
                        },
                    ],
                    devOps: [
                        {
                            name: 'Ubuntu/Arch',
                            level: 4,
                            expertise: 4,
                            icon: ['fab', 'ubuntu']
                        },
                        {
                            name: 'Nginx',
                            level: 4,
                            expertise: 5,
                            icon: '<svg xmlns="http://www.w3.org/2000/svg" role="img" viewBox="0 0 24 24" class="svg-inline--fa fa-symfony fa-w-16"><title/><path d="M12 0L1.605 6v12L12 24l10.395-6V6L12 0zm6 16.59c0 .705-.646 1.29-1.529 1.29-.631 0-1.351-.255-1.801-.81l-6-7.141v6.66c0 .721-.57 1.29-1.274 1.29H7.32c-.721 0-1.29-.6-1.29-1.29V7.41c0-.705.63-1.29 1.5-1.29.646 0 1.38.255 1.83.81l5.97 7.141V7.41c0-.721.6-1.29 1.29-1.29h.075c.72 0 1.29.6 1.29 1.29v9.18H18z"/></svg>'
                        },
                        {
                            name: 'Docker',
                            level: 3,
                            expertise: 2,
                            icon: ['fab', 'docker']
                        },
                    ]
                }

            }
        }
    }
</script>
